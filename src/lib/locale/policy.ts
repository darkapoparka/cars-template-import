...
export function createLocaleRouting<const L extends Language>(input: LocaleConfiguration<L>) {
	if (
		...
		!/^[a-z0-9][a-z0-9-]{0,127}$/.test(input.dealerId)
	) {
	...
	if (
		!Array.isArray(input.enabledLocales) ||
		!input.enabledLocales.length ||
	...
	if (
		!/^[A-Z]{3}$/.test(input.inventoryCurrency) ||
		!Intl.supportedValuesOf('currency').includes(input.inventoryCurrency)
	) {
		throw new Error('Invalid inventory currency');
	}
	if (
		!Number.isSafeInteger(input.preferenceMaxAge) ||
		input.preferenceMaxAge < 1 ||
		input.preferenceMaxAge > 31536000
	) {
	...
	if (
		!input.suggestedLanguages ||
		typeof input.suggestedLanguages !== 'object' ||
		Array.isArray(input.suggestedLanguages)
	)
	...
	for (const [country, locale] of Object.entries(suggestions)) {
		if (!isCountry(country) || !input.enabledLocales.includes(locale as L))
			throw new Error('Invalid country/language suggestion');
	}
	const directions = Object.freeze(
		Object.fromEntries(enabledLocales.map((locale) => [locale, languageRegistry[locale].direction]))
	) as Readonly<Record<L, Direction>>;
	const languageNames = Object.freeze(
		Object.fromEntries(enabledLocales.map((locale) => [locale, languageRegistry[locale].name]))
	...
	const contract = Object.freeze({
		...
		disabledLocales: Object.freeze(
			(Object.keys(languageRegistry) as Language[]).filter(
				(locale) => !enabledLocales.includes(locale as L)
			)
		)
	});
	const isLocale = (value: unknown): value is L =>
		typeof value === 'string' && enabledLocales.includes(value as L);
	...
	const formatPrice = (value: number, locale: L): string => {
		...
		return new Intl.NumberFormat(intlLocale(locale), {
			style: 'currency',
			currency: contract.inventoryCurrency,
			currencyDisplay: 'code',
			maximumFractionDigits: 0
		}).format(value);
	};
	function routeParts(pathname: string) {
		const mount = input.designBase ?? '';
		// Portable links preserve an explicit sibling design mount. The application reroute
		// separately checks that the resolved mount is its configured mount.
		const explicitDesign = pathname.match(/^\/variant-[23](?=\/|$)/)?.[0] ?? '';
		const base =
			mount && (pathname === mount || pathname.startsWith(mount + '/')) ? mount : explicitDesign;
	...
	function isResource(pathname: string) {
		...
		return (
			/^\/(?:admin|auth|api|_app|_next|assets|dealer-brand|dealer-inventory|brand|images|fonts|ingest)(?:\/|$)/.test(
				path
	...
	function localeHref(href: string, locale: L, defaultBase = ''): string {
		...
		if (!['', '/variant-2', '/variant-3', input.designBase ?? ''].includes(defaultBase))
			throw new Error('Invalid design mount');
		if (!href.startsWith('/') || href.startsWith('//') || /[\\\p{Cc}]/u.test(href)) return href;
...
export function createLocalePolicy<const L extends Language>(
	configuration: LocaleConfiguration<L>
) {
		...
		const choices = header
			...
			.map((part, index) => {
				...
				const q =
					weights.length === 0
						? 1
						: weights.length === 1 && /^q=(?:0(?:\.\d{0,3})?|1(?:\.0{0,3})?)$/.test(weights[0]!)
							? Number(weights[0]!.slice(2))
							: NaN;
				return { locale: tag.split('-')[0], q, index };
			})
			.filter(
				(choice): choice is { locale: L; q: number; index: number } =>
					isLocale(choice.locale) && Number.isFinite(choice.q) && choice.q > 0 && choice.q <= 1
			)
	...
	function resolveLocale(input: LocaleInput): ResolvedLocale<L> {
		...
		const explicit =
			routeParts(input.url.pathname).locale ?? (isLocale(queryLocale) ? queryLocale : null);
		const accepted = preferredLanguage(input.acceptLanguage);
		const countryLanguage = trustedCountry
			? contract.suggestedLanguages[trustedCountry]
			: undefined;
		...
		const locale =
			...
			(isLocale(savedLocale)
				? savedLocale
				: (accepted ?? suggestedLanguage ?? contract.defaultLocale));
		return {
			locale,
			source,
			country: isCountry(savedCountry) ? savedCountry : suggestedCountry,
			suggestedCountry,
			promptDismissed:
				cookieValue(input.cookie, 'cars_prompt') === contract.promptVersion ||
				(contract.promptVersion === 'v1' && cookieValue(input.cookie, 'cars_prompt') === '1')
		};
	}
	async function preferenceResponse(request: Request): Promise<Response> {
		...
		const fail = (status: number) =>
			new Response(JSON.stringify({ error: 'invalid_preference_request' }), { status, headers });
		...
		try {
			...
			if (type === 'application/json') {
				const keys = [...text.matchAll(/"((?:[^"\\]|\\.)*)"\s*:/g)].map(
					(match) => JSON.parse('"' + match[1] + '"') as string
				);
				if (new Set(keys).size !== keys.length) return fail(400);
			...
			if (
				...
				typeof data !== 'object' ||
				Array.isArray(data) ||
				Object.keys(data).some((key) => !['action', 'locale', 'country', 'returnTo'].includes(key))
			)
				return fail(400);
		} catch {
			return fail(400);
